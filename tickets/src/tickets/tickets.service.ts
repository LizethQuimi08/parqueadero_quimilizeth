...
export class TicketsService {
  ...
  constructor(
    ...
    this.personaUrl = this.configService.get<string>('MS_USUARIOS')!;
    this.espacioUrl = this.configService.get<string>('MS_ESPACIOS')!;
  ...
  async reservarEspacio(createTicketDto: CreateTicketDto): Promise<Ticket> {
    ...
    if (!persona) {
      throw new BadRequestException(
        `Usuario con DNI ${createTicketDto.dni} no encontrado`,
      );
    }

    if (persona.active === false) {
      throw new BadRequestException(
        `El usuario con DNI ${createTicketDto.dni} está inactivo`,
      );
  }
...
private async validarEspacioDisponible(
  ...
  const url = `${this.espacioUrl}/${idEspacio}/estado?estado=RESERVADO`;
  ...
  const url = `${this.espacioUrl}/${idEspacio}/estado?estado=OCUPADO`;
  await this.httpClient.patch(url, {});
  }
  ...
  const url = `${this.espacioUrl}/${idEspacio}/estado?estado=DISPONIBLE`;
  await this.httpClient.patch(url, {});
  }
